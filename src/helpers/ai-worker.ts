...
export function maximizePlay(
  ...
  const copyBoard = (gameGrid: counter[][]) => {
    const copyBoard = gameGrid.map((row) => [...row]);
    return copyBoard;
  };

  const boardIsFull = (gameGrid: counter[][]) => {
    for (let i = 0; i < 7; i++) {
  ...
  const findRowToLandCounter = (grid: counter[][], column: number): number => {
    let row: number = 0;
  ...
  const boardScore = (gameGrid: counter[][], gameScore: number): number => {
    let gameRows = 6;
    let gameColumns = 7;
